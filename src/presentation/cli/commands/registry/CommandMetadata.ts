 ...
 */
export type CommandCategory = "host" | "project-knowledge" | "relations" | "solution" | "work";
 ...
 */
export function parseCommandPath(filePath: string): {
  parent: string;
  subcommand: string;
} {
  ...
  if (parts.length !== 2) {
    throw new Error(
      `Invalid command filename: ${filePath}. Expected format: <parent>.<subcommand>.ts`
    );
  }
