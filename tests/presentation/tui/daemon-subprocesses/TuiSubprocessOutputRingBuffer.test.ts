...
import { TuiSubprocessOutputRingBuffer } from "../../../../src/presentation/tui/daemon-subprocesses/TuiSubprocessOutputRingBuffer.js";

describe("TuiSubprocessOutputRingBuffer", () => {
  it("splits output into non-empty retained lines and keeps only the newest entries", () => {
    const buffer = new TuiSubprocessOutputRingBuffer();
    const lines: string[] = [];
